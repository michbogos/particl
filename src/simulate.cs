		...
		for(int iter = 0; iter < 8; iter++){
			...
			for(int i = 0; i < 8; i++){
				...
				// memoryBarrierShared(); // Ensure change to foobar is visible in other invocations
  				// barrier();
				for(int other = 0; other < workgroups*workgroups*workgroups*8*8*8; other++){
				//for(int i = 0; i < 5; i++){
				...
				// 		for(int dz = -1; dz <= 1; dz++){
							...
							if(other != position){
								if(distance(positionBuffer[position], positionBuffer[other]) < 0.4){
				...
				// 	}
				// }
				positionBuffer[position]; // = positionBuffer2[newPosition];
				// memoryBarrierShared(); // Ensure change to foobar is visible in other invocations
  				// barrier();
			// }
			}
